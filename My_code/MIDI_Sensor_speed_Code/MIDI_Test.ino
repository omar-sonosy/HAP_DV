...
const unsigned sampleInterval = 10; // période d'échantillonnage (en ms)
...
bool detect_peak(int sensorIndex, int currentVoltage){
  if(currentVoltage>lastVoltage[sensorIndex]){
    current_state=true;
  }else{
    current_state=false;
  }
  if((current_state != last_state) && (!(abs(currentVoltage-lastVoltage[sensorIndex])<Noise_threshold))){
    return true;
  }else{
    return false;
  }
...
void readSensors(){
  for(int i = 0; i < numSensorsToDisplay; i++){
    ...
    int high_cutoff=map(1500, 0, bit, 0, tensionMaxADC);
    int low_cutoff=(1300, 0, bit, 0, tensionMaxADC);

    if(currentVoltage> high_cutoff || currentVoltage<low_cutoff){
